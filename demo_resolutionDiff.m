...
% % 转灰度
% map1=rgb2gray(imread('..\map_data\pair2\1.png','png'));
% map2=rgb2gray(imread('..\map_data\pair2\2.png','png'));
map1 = imresize(ori_map1,1.0);
% resize_times=1.2;
map2 = imresize(ori_map2,resize_times);
...
% % 选取harris-Laplace特殊点
% figure
% imshow(map1);hold on,'verbose'
result = vl_covdet(testMap1,...
    'Method','HarrisLaplace',...
    'BaseScale',2,...
    'OctaveResolution',3,...
    'DoubleImage',false);
% T = result(1:2) ;
...
% figure
% imshow(map2);hold on,'verbose'
result = vl_covdet(testMap2,...
    'Method','HarrisLaplace',...
    'BaseScale',2,...
    'OctaveResolution',3,...
    'DoubleImage',false);
