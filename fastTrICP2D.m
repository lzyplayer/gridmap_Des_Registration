function [R, t, TriKSI, minPhi] = fastTrICP2D( Model, Data,R, t,TrMin, TrMax, MoveStep)
...
Mns= createns(Model');
while ((CurrStep < MoveStep)&&(abs(minPhi-phiPre)>10^(-7)))
    ...
    [R, t, TCorr, TData] = CalRtPhi(Model, Data, SortCorrTD, TriKSI);   
    % return the MSE and the minimized function value
%     disp(['icp steps: ' num2str(CurrStep)]);
    CurrStep= CurrStep+1;   
end

disp(['icp iter times: ' num2str(CurrStep) ' minPhi:' num2str(minPhi)]);
